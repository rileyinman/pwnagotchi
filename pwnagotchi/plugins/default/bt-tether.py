...
class BTNap:
    ...
    def power(on=True):
        ...
        """
        logging.debug(f"[bt-tether] Changing bluetooth device to {str(on)}.")

        try:
            devs = list(BTNap.find_adapter())
            devs = dict((BTNap.prop_get(dev, 'Address'), dev) for dev in devs)
        except BTError as bt_err:
            logging.error(f"[bt-tether] Error: {bt_err}")
            return None

        for dev_addr, dev in devs.items():
            BTNap.prop_set(dev, 'Powered', on)
            logging.debug(f"[bt-tether] Set power of {dev.object_path} (addr {dev_addr}) to {str(on)}")
    ...
    def on_loaded(self):
                ...
                if 'enabled' in options and options['enabled']:
                    for device_opt in ['enabled', 'priority', 'scantime', 'search_order',
                        ...
                        if device_opt not in options or options[device_opt] is None:
                            logging.error(f"[bt-tether] Please specify the {device_opt} for device {device}.")
                            break
                    else:
                        if options['enabled']:
        ...
        if 'mac' in self.options:
            for opt in ['share_internet', 'mac', 'ip', 'netmask', 'interval']:
                if opt not in self.options or self.options[opt] is None:
                    logging.error(f"[bt-tether] Please specify the {opt} in your config.toml.")
                    return
        ...
        while self.running:
            ...
            for device in sorted_devices:
                ...
                try:
                    logging.debug(f"[bt-tether] Search {device.scantime} secs for {device.name}...")
                    dev_remote = bt.wait_for_device(timeout=device.scantime)
                    if dev_remote is None:
                        logging.debug(f"[bt-tether] Could not find {device.name}, try again in {device.interval} minutes.")
                        self.status = 'NF'
                        continue
                except Exception as bt_ex:
                    logging.error(bt_ex)
                ...
                if not paired:
                    if BTNap.pair(dev_remote):
                        logging.debug(f"[bt-tether] Paired with {device.name}.")
                    else:
                        logging.debug(f"[bt-tether] Pairing with {device.name} failed...")
                        self.status = 'PE'
                        continue
                else:
                    logging.debug('[bt-tether] Already paired.')
                ...
                logging.debug(f"[bt-tether] Try to create nap connection with {device.name}...")
                ...
                if success:
                    try:
                        ...
                        logging.debug(f"[bt-tether] Could not establish nap connection with {device.name}.")
                        continue
                    ...
                    logging.debug(f"[bt-tether] Created interface ({interface}).")
                    self.status = 'C'
                    any_device_connected = True
                ...
                logging.debug(f"[bt-tether] Adding IP to {interface}...")
                if not wrapped_interface.set_addr(addr):
                    self.status = 'AE'
                    logging.debug(f"[bt-tether] Could not add IP to {interface}.")
                    continue

                if device.share_internet:
                    if not connected_priorities or device.priority > max(connected_priorities):
                        logging.debug(f"[bt-tether] Set default route to {gateway} via {interface}.")
